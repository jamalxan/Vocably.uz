...
| B10 | Streak UTC bo'yicha, timezone/04:00 chegarasi yo'q | words/review/route.js                    | bug          | Yuqori    | 4    | Bajarildi |
| B11 | /api/stats/* va /api/dashboard yo'q                | -                                        | texnik qarz  | O'rta     | 3    | Ochiq  |
| B12 | Test runner sozlanmagan                            | package.json                             | texnik qarz  | O'rta     | 4    | Bajarildi (Vitest) |
...
Also, off-spec but user-requested same day: `/users` and `/stats` admin commands added to the Telegram webhook (gated by `TELEGRAM_ADMIN_CHAT_ID` env var), plus a standalone `scripts/broadcast-telegram.mjs` for one-off update announcements to all linked users. Not part of the spec's phase plan — flagging here so it isn't lost. The app has no `email` field anywhere (phone-only login), so `/users` reports name + phone instead.

## 10. FAZA 2 report-back

**Status: FAZA 2 complete** on `feat/phase-2-srs-db`, branched from `feat/phase-1-critical-fixes` (not yet merged to `master` — see open question below). Per §8's decision, this stays on MongoDB with a **restructured embedded shape** for word stats plus one genuinely new top-level collection, not a Postgres migration and not a full move to separate `user_word_state`/`study_sessions` collections:

- **`src/lib/srs.ts`** (new, TypeScript): the real SM-2-variant engine from spec §6.2 — learning steps, ease factor, lapses, leech threshold (8), interval fuzz (±5%), 365-day cap. Pure functions, no I/O. 28 Vitest unit tests (`src/lib/srs.test.ts`), including the timezone/04:00-cutoff edge cases. Also added Vitest itself (B12 — no test runner existed at all).
- **`WordStatsSchema`** (`src/lib/models.js`): added `srsState`, `ease`, `intervalDays`, `learningStep`, `lapses`, `reps`, `isLeech`. Kept `level`/`nextReview`/`correct`/`wrong` — `level` is now *derived* from `intervalDays` (`levelFromIntervalDays`, matches spec §5.2's mastery thresholds) rather than being its own counter, so the existing "O'zlashtirilgan" (mastered) UI reads in `SpacedRepetition.jsx` keep working unchanged.
- **Legacy migration is on-read, not a batch script**: `PATCH /api/words/review` detects a word that has real history under the old flat-lookup fields (`level`/`correct`/`wrong`) but has never touched the new engine fields, and one-time-converts it via `migrateLegacyCard()` on its next review. No separate migration job, no downtime, no risk of double-migrating (covered by a dedicated test).
- **`User.timezone`** (default `Asia/Tashkent`) — not a full `user_settings` collection (that's Faza 6/11-scale scope), just the one field needed to fix B10 correctly.
- **B10 fixed**: streak is now computed via `computeStreakUpdate()` using the user's timezone with a 04:00 local cutoff, replacing the old raw-UTC `toISOString().slice(0,10)` comparison.
- **New `ReviewEvent` collection** (top-level, append-only): one row per answer (`userId`, `categoryId`, `wordId`, `mode`, `rating`, `isCorrect`, before/after state+interval+ease, `reviewedAt`), indexed on `(userId, reviewedAt)`. This is the Mongo-adapted equivalent of spec §6.1's `review_events` table — kept out of the `User` document on purpose so Faza 3's dashboard/stats endpoints have something to query without bloating the payload every `/api/words` fetch already returns. Write failures are logged, not thrown — a lost audit row must never block the user's actual review from saving.
- **`AppContext.reviewWord`** (frontend): removed the duplicate client-side flat-lookup prediction (it would now drift from the real ease-based server result). Replaced with a lightweight optimistic "not due right now" placeholder, then the server's authoritative `stats` overwrites it once the PATCH resolves.
- **Rating plumbing**: the endpoint now accepts an optional `rating` (1-4) and falls back to `ratingFromOutcome(correct, responseMs)` when the caller (today, all of them) only sends the old boolean `correct`. This means every study mode already gets real SRS behavior with zero UI changes; Faza 5's 4-button flashcard redesign just needs to start sending a real `rating` instead of relying on the fallback.
- **Deliberately not touched**: B3 (the `POST /api/words` full-array overwrite race). It's a category/word-CRUD correctness issue, not an SRS/DB-model one, and fixing it means refactoring `AppContext`'s `syncData` call sites broadly (add-word, add-category, rename, etc.) — different blast radius than this phase. Left open, still logged below.

**Environment note, not a code issue**: the dev machine's `C:` drive was down to ~51 MB free during this phase, which made `next build` fail on webpack's on-disk cache (`ENOSPC`) until `.next` was cleared. Build/lint/type-check/test all pass now, but worth the user's attention — 51 MB free on a 200 GB drive will keep causing random build/npm failures until real space is freed.

**Open question**: `feat/phase-2-srs-db` currently branches off the unmerged `feat/phase-1-critical-fixes` rather than `master`, since Phase 1 hasn't been reviewed/merged yet. Fine for now, but the PR chain (phase-1 → master, then phase-2 → master or → phase-1) needs a decision before either actually merges.
