...
// (Writing uchun bir xil qoida). `/app/oqish-beta/page.jsx` bilan bir xil
// naqsh — `?testId=` shu bitta ExamTest hujjatidagi "writing" bo'limi uchun
// attempt yaratadi (scripts/seed-exam-test.mjs Reading bilan bir vaqtda
// Writing bo'limini ham yozadi).
function YozishBetaEntryInner() {
  ...
  const start = async () => {
    ...
    setStarting(true);
    try {
      const { attemptId } = await createAttempt(testId.trim(), 'writing');
      router.push(`/app/yozish-beta/${attemptId}`);
    } catch {
  ...
  return (
    <div className="max-w-md mx-auto p-6 sm:p-10">
      <h1 className="text-lg font-bold text-ink mb-1">Writing Beta — Exam Engine v1</h1>
      <p className="text-sm text-muted mb-6">
        TZ-vocably-v2.md &quot;IELTS CD Exam Engine&quot; Faza 2 sinovi. Avval{' '}
      ...
      >
        ...
        Writing bo&apos;limini boshlash
      </button>
    </div>
  );
}

export default function YozishBetaEntryPage() {
  return (
    <Suspense fallback={<div className="p-8 text-sm text-muted">Yuklanmoqda...</div>}>
      <YozishBetaEntryInner />
    </Suspense>
  );
}
