...
// (Listening uchun bir xil qoida). `/app/oqish-beta/page.jsx` bilan bir xil
// naqsh — `?testId=` shu bitta ExamTest hujjatidagi "listening" bo'limi
// uchun attempt yaratadi.
function TinglashBetaEntryInner() {
  ...
  const start = async () => {
    ...
    setStarting(true);
    try {
      const { attemptId } = await createAttempt(testId.trim(), 'listening');
      router.push(`/app/tinglash-beta/${attemptId}`);
    } catch {
  ...
  return (
    <div className="max-w-md mx-auto p-6 sm:p-10">
      <h1 className="text-lg font-bold text-ink mb-1">Listening Beta — Exam Engine v1</h1>
      <p className="text-sm text-muted mb-6">
        TZ-vocably-v2.md &quot;IELTS CD Exam Engine&quot; Faza 2 sinovi. Naushnik taqing — audio ATAYLAB haqiqiy nutq emas,
        faqat sinov toni (TTS quvuri hali yo&apos;q). Avval{' '}
      ...
      >
        ...
        Listening bo&apos;limini boshlash
      </button>
    </div>
  );
}

export default function TinglashBetaEntryPage() {
  return (
    <Suspense fallback={<div className="p-8 text-sm text-muted">Yuklanmoqda...</div>}>
      <TinglashBetaEntryInner />
    </Suspense>
  );
}
