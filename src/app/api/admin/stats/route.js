...
// Umumiy (foydalanuvchiga xos bo'lmagan) agregat hisob — admin panel har ochilganda/
// yangilanganda va Telegram /statistika orqali chaqiriladi, lekin soniyama-soniya aniqlik
// shart emas, shuning uchun qisqa TTL bilan keshlanadi (src/lib/cache.js).
async function computeAdminStats() {
  ...
  return {
    ...
    totalMessages,
    openReports,
    typeBreakdown: Object.fromEntries(messagesByType.map((r) => [r._id, r.count])),
  };
}
...
export async function GET(req) {
  try {
    ...
    const stats = await cached('admin:stats', computeAdminStats, 60_000);
