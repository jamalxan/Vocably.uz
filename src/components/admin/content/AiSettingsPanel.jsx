...
import { Loader2, Save, DollarSign, ShieldAlert } from 'lucide-react';
...
export default function AiSettingsPanel() {
  ...
  // AI-02 — texnik maydonlar (model/fallback/temperature/maxTokens/costCap)
  // ilgari HAR BIR adminga ochiq edi. Endi ular "Expert rejim" ortida
  // yashiringan — oddiy admin faqat vazifa nomini (TASK_KEY_LABEL) ko'radi.
  // Bu sof UI ko'rinish holati, hech qaysi tabiiy server-side admin
  // preference joyi yo'q (bu komponent har safar /api/admin/ai/config'dan
  // qayta yuklanadi, foydalanuvchiga xos sozlama saqlash joyi yo'q), shuning
  // uchun ataylab local state — sahifa yangilanganda standart OFF holatga
  // qaytadi.
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            {expertMode ? 'Har bosqich uchun model, fallback zanjiri va xarajat chegarasi.' : 'Har bosqich qanday ishlayotgani.'}
          </p>
        </div>
        <button
          type="button"
          ...
          title="Model, temperature, maxTokens va xarajat chegarasi kabi texnik maydonlarni ko'rsatish"
          className={`flex items-center gap-2 px-3 py-1.5 min-h-11 md:min-h-0 rounded-xl border text-xs font-semibold transition-colors flex-shrink-0 ${
            expertMode ? 'border-accent bg-accent-soft text-accent' : 'border-border bg-surface text-muted hover:text-ink'
          }`}
        >
          <ShieldAlert size={13} />
          Expert rejim
          <span
            aria-hidden="true"
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${expertMode ? 'bg-accent' : 'bg-border'}`}
          >
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${expertMode ? 'translate-x-5' : 'translate-x-1'}`} />
          </span>
        </button>
      </div>
            ...
            <div key={c.taskKey} className="px-4 py-3.5 bg-surface border border-border rounded-xl">
              ...
              {expertMode ? (
                ...
                <p className="text-xs text-muted">
                  Texnik sozlamalar (model, temperature, maxTokens, xarajat chegarasi) — "Expert rejim"ni yoqing.
                </p>
              )}
            </div>
          );
