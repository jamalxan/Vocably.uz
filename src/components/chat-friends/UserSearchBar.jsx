          ...
          {result ? (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-slate-800 truncate">@{result.username}</p>
                {result.name && <p className="text-xs text-slate-400 truncate">{result.name}</p>}
              </div>
              {opening ? <Loader2 size={14} className="animate-spin text-indigo-500" /> : (
                <span className="text-xs text-indigo-600 font-medium flex-shrink-0">Yozish</span>
              )}
            </button>
          ) : (
            <p className="text-xs text-slate-400 text-center">Foydalanuvchi topilmadi</p>
          )}
        </div>
      )}
    </div>
